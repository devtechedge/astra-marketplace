...
```bash
cp .env.example .env
# edit DATABASE\_URL or run local Postgres
npm run db:generate
npm run db:push
npm run db:seed
```
...
4. For database-backed production mode, add a hosted PostgreSQL database such as Vercel Postgres, Neon, Supabase or Railway and set `DATABASE\_URL`.
